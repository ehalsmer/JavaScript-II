...
function concat(item, i, array){ 
    fullName.push(`${array[i].first_name} ${array[i].last_name}`);
}

runners.forEach(concat);
...
let ticketPriceTotal = runners.reduce((accumulator, runner)=>{return accumulator += runner.donation}, 0);
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1: Make a list of people who donated more than 100, using .filter()
let largeDonations = runners.filter((runner)=>{return runner.donation >= 100});
console.log(largeDonations);

// Problem 2: Create an array of just emails, to send out a newsletter
let emails = runners.map((runner)=>{return runner.email});
console.log(emails);

// Problem 3: Create an array of last names, sorted alphabetically
let lastNamesSorted = runners.map((runner)=>runner.last_name).sort();
